class Tweets::MakeStemJob < ApplicationJob
  queue_as :default
  include SettingsHelper

  def perform(discussion:)
    return if discussion.tweet.present?
    ...
    question = <<~QUESTION
        ...
        - It is your job to write a funny 200 character `tweet` about this story.
    ...
    if response["error"].present?
      Tweet.create(discussion:, processed: true, invalid_json:)
    else
      Tweet.create(discussion:, stem: response["choices"][0]["message"]["content"], processed: true, invalid_json:)
    end
  end
